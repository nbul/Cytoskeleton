clear object_double  
image_original_double_actin = double(im2uint16(Image_actin));
%%  Assign memory.

a_added_norm = zeros(45,numel(b_valid)+1);
a_added_norm(:,1) = bincenter;
...
for k = 1:numel(b_valid)
    ...
    object_double = image_original_double_actin .*...
    ...
    for i=1:(length(binrange)-1)
        a_added(i,1) = sum(mxd_indexed(mxd_indexed(:,1) == i,2));
    end
   
    a_added_norm(:,k+1) = a_added/sum(a_added);
end
