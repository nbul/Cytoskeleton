...
actin_area = zeros(1, numel(b_valid));
actin_signal = zeros(1, numel(b_valid));
...
for k = 1:numel(b_valid)
    ...
    to_analyse_o = regionprops(selected_signal, Image_actin,'PixelValues');
    ...
    % Signal Area
    actin_area(k) = num_pixvalues_c / (num_pixvalues_c + num_pixvalues_back_c);  
    actin_signal(k) = mean(to_analyse_o.PixelValues(to_analyse_o.PixelValues(:, 1) ~= 0,1));
end