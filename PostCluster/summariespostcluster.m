...
result(:,1) = (0.92:0.005:0.99)';
result(:,2) = SD';
result(isnan(result(:,2)) == 1,:) = [];

cd(sum_dir);
headers = {'Eccentricity','MTSD'};
csvwrite_with_headers('MTSDpulled.csv',result,headers);

%% Pulled but not corrected for cell orientation
cd(dist_dir);
MTSDpulled(:,1) = csvread('1_distribution.csv',0,0, [0 0 44 0]);
Eccpulled = zeros(1,1);
counter_cells = 0;
...
cd(sum_dir);
headers = {'Eccentricity','MTSD'};
csvwrite_with_headers('MTSDpullednotcorrected.csv',result,headers);
